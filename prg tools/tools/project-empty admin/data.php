...
function rename_file($image_type,$num,$model)
				{
						...
						return $new_image_name=$model.$num.'_'.uniqid().'.'.$extension;	
				}
...
function timecreate($time)
 ...
 if($time>12)
   ...
   //validation
function validation($type,$file_name)
   
    {
      ...
      if ((($_FILES[$file_name]["type"] == "image/gif") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/png")) && in_array($extension, $allowedExts))
       {
